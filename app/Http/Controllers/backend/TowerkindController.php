...
// контроллер модели
class TowerkindController extends Controller
{
    ...
    public function index()
    {
        ...
        // открыть вюшку
        return view('backend.towerkind.index');
    }
    ...
    public function vueSave(Request $request)
    {
        ...
        if (isset($getSpravRowContent['id']) and $getSpravRowContent['id'] > 0) {
            $modelSave = Towerkind::find($getSpravRowContent['id']);
        } else {
            $modelSave = new Towerkind;
        }
        ...
        $modelSave = Towerkind::find($newID);
    ...
    public function destroy($getID)
    {
        ...
        $this->commonCrudService->destroy('Towerkind', $getID);
    }
