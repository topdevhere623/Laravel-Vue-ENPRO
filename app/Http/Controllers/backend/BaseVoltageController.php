...
// контроллер модели
class BaseVoltageController extends Controller
{
    ...
    public function index()
    {
        ...
        // $content = BaseVoltage::paginate($paginate);

        // открыть вюшку
        return view('backend.basevoltage.index');
    }
    public function vueIndex(Request $request)
    {
        ...
        $myReturn = BaseVoltage::when(isset($getFilterName), function ($query) use ($getFilterName) {
    ...
    public function edit($id = null)
    {
        // контент
        if ($id) {
            $content = BaseVoltage::findOrFail($id);
        } else {
            $content = new BaseVoltage;
        }

        // справочники и другие дополнительные сведения

        // открыть вьюшку
        return view('backend.basevoltage.edit', compact('content'));
    }

    // сохранение данных
    public function update($id = null, Request $request)
    {
        $bool = $this->commonCrudService->store('BaseVoltage', $id, $request);
        // редирект
        return redirect(route('basevoltage.index'));
    }
    ...
    public function destroy($id)
    {
        ...
        $bool = $this->commonCrudService->destroy('BaseVoltage', $id);
        // редирект
