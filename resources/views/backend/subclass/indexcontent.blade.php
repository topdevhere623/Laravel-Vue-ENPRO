...
<div class="page-header">
    {{-- заголовок --}}
    <h2 class="page-title">{{ App\Models\Subclass::title2 }}</h2>

    {{-- хлебные крошки --}}
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('admin') }}">Главная</a></li>
        <li class="breadcrumb-item active">{{ App\Models\Subclass::title2 }}</li>
    </ol>

    {{-- действия на странице --}}
    <div class="page-header-actions">
        <a href="{{ route('subclass.edit') }}" class="btn btn-lg btn-icon btn-primary btn-round" data-toggle="tooltip"
                    ...
                    <table class="table table-hover" data-plugin="selectable" data-row-selectable="false">
                        ...
                        @foreach($content as $item)
                            <tr>
                                ...
                                <td>{{ $item->id }}
                                <td>
                                    <a href="{{ route('subclass.edit',['id'=>$item->id]) }}">
                                        {{ $item->name }}
                                    </a>
                                <td>
                                {{ $item->parent }}
                                <td class="text-nowrap">
                                    <form action="{{ route('subclass.destroy',['id'=>$item->id]) }}" method="post">
                                        {{ @csrf_field() }}
                                        {!! method_field('delete') !!}
