...
<div class="page-header">
    ...
    <div class="page-header-actions">
        <a href="{{ route('crossingtype.edit') }}" class="btn btn-lg btn-icon btn-primary btn-round" data-toggle="tooltip"
                ...
                <div class="col-lg-12">
                    ...
                    <table class="table table-hover" data-plugin="selectable" data-row-selectable="false">
                        ...
                        <tr>
                            ...
                            <th>ID
                            <th>{{ App\Models\Crossingtype::title1 }}
                        ...
                        @foreach($content as $item)
                            <tr>
                                ...
                                <td>{{ $item->id }}
                                <td>
                                    <a href="{{ route('crossingtype.edit',['id'=>$item->id]) }}">
                                        {{ $item->name }}
                                    </a>
                                <td>
                                    <li class="list-inline-item mr-5">
                                        <input type="checkbox" data-plugin="switchery" data-size="small" @if ($item->status) checked @endif onchange="myFunAjaxChangeField( {{ $item->id }}, 'Crossingtype')" name="status">
                                    </li>
                                <td class="text-nowrap">
                                    <form action="{{ route('crossingtype.destroy',['id'=>$item->id]) }}" method="post">
                                        {{ @csrf_field() }}
                                        {!! method_field('delete') !!}
