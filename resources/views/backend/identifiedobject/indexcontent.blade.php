...
<div class="page-header">
    ...
    <div class="page-header-actions">
        <a href="{{ route('yandex_map.index',['model' => 'identifiedobject']) }}" class="btn btn-lg btn-icon btn-success" data-toggle="tooltip"
           data-original-title="На карте">
            <i class="icon md-google-maps" aria-hidden="true"></i>
        </a>
        <a href="{{ route('identifiedobject.edit') }}" class="btn btn-lg btn-icon btn-primary btn-round" data-toggle="tooltip"
                ...
                <div class="col-lg-12">
                    ...
                    <table class="table table-hover" data-plugin="selectable" data-row-selectable="false">
                        ...
                        <tr>
                            ...
                            <th>ID
                            <th>{{ App\Models\Identifiedobject::title1 }}
                            <th>ЛЭП
                            <th>Дисп.имя
                            <th>Дисп.№
                            <th>Адрес
                        ...
                        @foreach($content as $item)
                            <tr>
                                ...
                                <td>{{ $item->id }}
                                <td>
                                    <a href="{{ route('identifiedobject.edit',['id'=>$item->id]) }}">
                                        {{ $item->id }}
                                    </a>
                                <td>{{ $item->acline->identifiedobject->name }}
                                <td>{{ $item->name }}
                                <td>{{ $item->localname }}
                                <td>{{ $item->address }}
                                <td>{{ $item->classname->name }}
                                <td>{{ $item->subclass_id }}
                                <td>{{ $item->asset_id }}
                                <td>{{ $item->keylink }}
                                <td>{{ $item->lat }}
                                <td>{{ $item->long }}
                                <td>{{ $item->basevoltage->name }}
                                <td>{{ $item->phaseno }}
                                <td class="text-nowrap">
                                    <form action="{{ route('identifiedobject.destroy',['id'=>$item->id]) }}" method="post">
                                        {{ @csrf_field() }}
                                        {!! method_field('delete') !!}
