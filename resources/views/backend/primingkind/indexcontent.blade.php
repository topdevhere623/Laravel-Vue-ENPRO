...
<div class="page-header">
    {{-- заголовок --}}
    <h2 class="page-title">{{ App\Models\Primingkind::title2 }}</h2>

    {{-- хлебные крошки --}}
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('admin') }}">Главная</a></li>
        <li class="breadcrumb-item active">{{ App\Models\Primingkind::title2 }}</li>
    </ol>

    {{-- действия на странице --}}
    <div class="page-header-actions">
        <a href="{{ route('primingkind.edit') }}" class="btn btn-lg btn-icon btn-primary btn-round" data-toggle="tooltip"
                ...
                <div class="col-lg-12">
                    ...
                    <table class="table table-hover" data-plugin="selectable" data-row-selectable="false">
                        ...
                        <tr>
                            ...
                            <th>ID
                            <th>{{ App\Models\Primingkind::title1 }}
                            <th>Описание
                        ...
                        @foreach($content as $item)
                            <tr>
                                ...
                                <td>{{ $item->id }}
                                <td>
                                    <a href="{{ route('primingkind.edit',['id'=>$item->id]) }}">
                                        {{ $item->name }}
                                    </a>
                                <td>
                                {{ $item->description }}
                                <td class="text-nowrap">
                                    <form action="{{ route('primingkind.destroy',['id'=>$item->id]) }}" method="post">
                                        {{ @csrf_field() }}
                                        {!! method_field('delete') !!}
