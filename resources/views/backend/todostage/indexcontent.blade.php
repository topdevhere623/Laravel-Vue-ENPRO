...
<div class="page-header">
    {{-- заголовок --}}
    <h2 class="page-title">{{ App\Models\TodoStage::title2 }}</h2>

    {{-- хлебные крошки --}}
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('admin') }}">Главная</a></li>
        <li class="breadcrumb-item active">{{ App\Models\TodoStage::title2 }}</li>
    </ol>

    {{-- действия на странице --}}
    <div class="page-header-actions">
        <a href="{{ route('todostage.edit') }}" class="btn btn-lg btn-icon btn-primary btn-round" data-toggle="tooltip"
                ...
                <div class="col-lg-12">
                    ...
                    <table class="table table-hover" data-plugin="selectable" data-row-selectable="false">
                        ...
                        <tr>
                            ...
                            <th>ID
                            <th>{{ App\Models\TodoStage::title1 }}
                        ...
                        @foreach($content as $item)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('todostage.edit',['id'=>$item->id]) }}">
                                        {{ $item->id }}
                                    </a>
                                <td>
                                    <a href="{{ route('todostage.edit',['id'=>$item->id]) }}">
                                ...
                                <td>
                                    <li class="list-inline-item mr-5">
                                        <input type="checkbox" data-plugin="switchery" data-size="small" @if ($item->status) checked @endif onchange="myFunAjaxChangeField( {{ $item->id }}, 'TodoStage')" name="status"
                                               @if (
                                ...
                                <td class="text-nowrap">
                                    <form action="{{ route('todostage.destroy',['id'=>$item->id]) }}" method="post">
                                        {{ @csrf_field() }}
                                        {!! method_field('delete') !!}
