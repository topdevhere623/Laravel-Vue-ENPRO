...
<div class="page-header">
    {{-- заголовок --}}
    <h2 class="page-title">{{ App\Models\Tasktype::title2 }}</h2>

    {{-- хлебные крошки --}}
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('admin') }}">Главная</a></li>
        <li class="breadcrumb-item active">{{ App\Models\Tasktype::title2 }}</li>
    </ol>

    {{-- действия на странице --}}
    <div class="page-header-actions">
        <a href="{{ route('tasktype.edit') }}" class="btn btn-lg btn-icon btn-primary btn-round" data-toggle="tooltip"
                    ...
                    <table class="table table-hover" data-plugin="selectable" data-row-selectable="false">
                        ...
                        @foreach($content as $item)
                            <tr>
                                ...
                                {{ $item->id }}
                                <td>
                                    <a href="{{ route('tasktype.edit',['id'=>$item->id]) }}">
                                        {{ $item->title }}
                                    </a>
                                <td>
                                {{ $item->object }}
                                <td class="text-nowrap">
                                    <form action="{{ route('tasktype.destroy',['id'=>$item->id]) }}" method="post">
                                        {{ @csrf_field() }}
                                        {!! method_field('delete') !!}
