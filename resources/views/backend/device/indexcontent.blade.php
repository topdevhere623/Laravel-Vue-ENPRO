...
<div class="page-header">
    {{-- заголовок --}}
    <h2 class="page-title">{{ App\Models\Device::title2 }}</h2>

    {{-- хлебные крошки --}}
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('admin') }}">Главная</a></li>
        <li class="breadcrumb-item active">{{ App\Models\Device::title2 }}</li>
    </ol>

    {{-- действия на странице --}}
    <div class="page-header-actions">
        <a href="{{ route('device.edit') }}" class="btn btn-lg btn-icon btn-primary btn-round" data-toggle="tooltip"
                        ...
                        @foreach($content as $item)
                            <tr>
                                ...
                                <td class="text-nowrap">
                                    <form action="{{ route('device.destroy',['id'=>$item->id]) }}" method="post">
                                        {{ @csrf_field() }}
                                        {!! method_field('delete') !!}
