  ...
  methods: {
    setCurrentPosition() {
      if (this.itemData.coords.length === 0) {
        const bottomTiresList = this.elementsList.filter(el => this.itemData.type === el.type);
        const idsList = bottomTiresList.map(el => el.id);
        ...
        if (index > 0) {
          this.x1 = bottomTiresList[index - 1].coords[1].x + this.defaultWidth;
          this.x2 = this.x1 + this.defaultWidth;
        }
