  ...
  <section>
    ...
    <div class="table-bottom-bar">
      ...
      <div class="left">
        ...
        <a class="link-icon" @click="funSelectedRows('status')"
          >Сменить статус</a
        >
        <a class="link-icon" @click="funSelectedRows('delete')"
          >Удалить выбранные</a
        >
      </div>
...
export default {
  ...
  methods: {
    ...
    async funLoadContent(page = 1) {
      ...
      await axios
        .post("/api/towerconstructionkindVueIndex", form)
        .then((response) => {
          // запрос прошел
          // для отладки
    ...
    async funSelectedRows(getRegim) {
      ...
      await axios
        .post("/api/towerconstructionkindVueSelectedRows", form)
        .then((response) => {
          // запрос прошел
          // для отладки
    ...
    funEditBegin(getIndex) {
      ...
      for (let key in this.modelData.data[getIndex]) {
        this.modelData.data[getIndex].oldValue[key] =
          this.modelData.data[getIndex][key];
      }
    ...
    async funSave(getIndex = null) {
      ...
      form.append(
        "spravRowContent",
        JSON.stringify(this.modelData.data[getIndex])
      );

      axios
        .post("/api/towerconstructionkindVueSave", form)
        .then((response) => {
          // запрос прошел
          // для отладки
    ...
    funMakeCopy(getIndex) {
      ...
      this.modelData.data[myFindRowN].status =
        this.modelData.data[getIndex].status;
    ...
    funRowIsEmpty(getIndex) {
      ...
      if (
        this.modelData.data[getIndex].id == null &&
