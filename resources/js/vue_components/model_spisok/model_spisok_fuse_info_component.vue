  ...
  <section>
    ...
    <div class="col-md-12 filter-container collapse" id="collapseTableFilter">
      <div class="filter-bar row">
        ...
        <div class="input-range-slider col-md-6">
          <div class="form-group mb-0">
            <label for="#nom-min">Номинальный ток предохранителя</label>
            <div class="d-flex r-input-container mb-2">
              <div class="input-group form-field">
                <input
                  type="text"
        ...
        <div class="input-range-slider col-md-6">
          <div class="form-group mb-0">
            <label for="#nom-min">Номинальный ток отключения</label>
            <div class="d-flex r-input-container mb-2">
              <div class="input-group form-field">
                <input
                  type="text"
                  class="form-control text-field"
                  v-model.number="filter.breakingCapacity.min"
                  id="nom-min"
                  placeholder="Минимальное"
                  aria-describedby="inputGroupPrepend5"
                  required
              ...
              <div class="input-group form-field">
                <input
                  type="text"
                  class="form-control text-field"
                  v-model.number="filter.breakingCapacity.max"
                  id="nom-max"
                  placeholder="Максимальное"
        ...
        <tbody>
          ...
          <tr v-for="(item, index) in modelData.data" :key="index">
            ...
            <td class="text-center">
              <a @click="saveQuery" :href="`fuse_info/edit/${item.id}`">
                {{ item.id }}
              </a>
            </td>
            <td>
              <a
                v-if="item.AssetInfo.CatalogAssetType.IdentifiedObject.name"
                @click="saveQuery"
                :href="`fuse_info/edit/${item.id}`"
              >
            ...
            <td>
              <a @click="saveQuery" :href="`fuse_info/edit/${item.id}`">
                {{
                  item.breakingCapacity.value !== null
                    ? `${item.breakingCapacity.value} кA`
                    : ""
                }}
              </a>
            </td>
            <td>
              <a @click="saveQuery" :href="`fuse_info/edit/${item.id}`">
                {{
                  item.enproBreakerKind !== null
                    ? item.enproBreakerKind.ru_value
                    : ""
                }}
              </a>
            </td>
  ...
  data() {
    return {
      ...
      contentTh: {
        ...
        3: {
          name: "Номинальный ток предохранителя",
          sortFieldName: "ratedCurrent",
          width: "auto",
        },
        4: {
          name: "Номинальный ток отключения",
          sortFieldName: "breakingCapacity",
          width: "auto",
        },
        5: {
          name: "Принцип гашения дуги",
          sortFieldName: "enproBreakerKind",
          width: "auto",
        },
      },
      selectedAll: false,
  ...
  methods: {
    ...
    funFiltering() {
      ...
      if (
        ...
        this.filter.breakingCapacity.min !== null ||
        this.filter.breakingCapacity.max !== null ||
        this.filter.enproBreakerKindId?.length > 0
      ) {
    ...
    funLoadAll() {
      if (localStorage.getItem("table_search")) {
        if (localStorage.getItem("spisok_model") === "FuseInfo") {
    ...
    funLoadContent(page = 1) {
      ...
      let url = `/api/modelName/FuseInfo/switchInfo?page=${currentPage}`;
      ...
      if (this.filter.breakingCapacity.min) {
        url += "&breakingCapacityMin=" + this.filter.breakingCapacity.min;
      }
      if (this.filter.breakingCapacity.max) {
        url += "&breakingCapacityMax=" + this.filter.breakingCapacity.max;
      }

      if (this.filter.enproBreakerKindId?.length > 0) {
        this.filter.enproBreakerKindId.forEach((mid) => {
          url += "&enproBreakerKindId[]=" + mid;
        });
      }
      axios
        .get(url)
    ...
    async funSelectedRows() {
      ...
      if (Object.keys(this.selectedRows).length === 1) {
        await axios
          .delete(`/api/modelName/FuseInfo/switchInfo/${this.selectedRows[0]}`)
          .then((response) => {
            this.funLoadAll();
          })
          .catch((error) => {
    ...
    saveQuery() {
      ...
      localStorage.setItem("spisok_model", "FuseInfo");
    },
  },
};
</script>
