  ...
  <section>
      ...
      >
        ...
        <tr v-for="item in modelData.data" :key="item.id">
          ...
          <td class="text-center">
            <a :href="'/admin/towerinfo/edit/' + item.id">
              {{ item.id }}
            </a>
          </td>
          <td>
          ...
          <td>
            <a
              ...
              :href="
                  '/public/uploads/models/towerinfo/' + item.id + '/' + item.img
                "
              target="_blank"
            >
              <img
                v-if="item.img !== '' && item.img != null"
                :src="
                    '/public/uploads/models/towerinfo/' +
                    item.id +
                    '/' +
                    item.img
                  "
          ...
          <td class="text-center">
            {{ item.towerconstructions.length }}
          </td>
          <td class="text-center">
    ...
    <div class="table-bottom-bar">
      ...
      <div class="left">
        ...
        <a class="link-icon" @click="funSelectedRows('copy')">Сделать копию</a>
...
<script>
  ...
  export default {
    props: {},
    mixins: [spisokLiveSearch, modelSpisok],
    data() {
      return {
        loading: false,
        errored: false,
        ...
        contentTh: {
          ...
          6: {
            name: "Материал",
            sortFieldName: "towermaterial_name",
            width: 75,
          },
          7: {
            name: "Назначение",
            sortFieldName: "towerkind_name",
            width: 75,
          },
          8: {
            name: "Компонент",
    ...
    methods: {
      ...
      funLoadContent(page = 1) {
        ...
        axios
          .post("/api/towerinfoVueIndex", form)
          .then((response) => {
            // для отладки
      ...
      async funSelectedRows(getRegim) {
        ...
        await axios
          .post("/api/towerinfoVueSelectedRows", form)
          .then((response) => {
            // запрос прошел
            // для отладки
