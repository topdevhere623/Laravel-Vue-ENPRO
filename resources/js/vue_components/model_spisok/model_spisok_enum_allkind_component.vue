        ...
        <tbody>
          ...
          <tr v-for="(item, index) in modelData.data" :key="index">
            ...
            <td class="text-center">
              <a :href="`all_enum_kind/${getModelName}/edit/${item.id}`">
                {{ item.id }}
              </a>
            </td>
            <td class="text-center">
            ...
            <td class="text-left">
              <a
                v-if="item.ru_value"
                :href="`all_enum_kind/${getModelName}/edit/${item.id}`"
              >
                {{ item.ru_value }}
              </a>
            </td>
...
export default {
  name: "model_spisok_enum_allkind_componnet",
  props: {
    getUserRole: {
      required: true,
      type: String,
  ...
  data() {
    return {
      ...
      contentTh: {
        ...
        1: {
          name: "Условное обозначение",
          sortFieldName: "literal",
          width: "auto",
        },
        2: {
  ...
  methods: {
    funLoadContent(page = 1) {
      ...
      axios
        .get(`/api/all_enum_kind/model/${this.getModelName}/`, {
          params,
        })
        .then((response) => {
          // для отладки
    ...
    async funSelectedRows() {
      ...
      await axios
        .post(`/api/all_enum_kind/model/${this.getModelName}/delete`, params)
        .then((response) => {
          // запрос прошел
          // для отладки
