        ...
        <tbody>
          ...
          <tr v-for="(item, index) in modelData.data" :key="index">
            ...
            <td class="text-center">
              <a :href="`crossing/edit/${item.id}`">
                {{ item.id }}
              </a>
            </td>
            <td class="text-center">
              <a
                v-if="item.crossingtype.name"
                :href="`crossing/edit/${item.id}`"
              >
            ...
            <td class="text-center">
              <a
                v-if="item.identifiedobject.name"
                :href="`crossing/edit/${item.id}`"
              >
...
export default {
  ...
  data() {
    return {
      ...
      contentTh: {
        ...
        1: {
          name: this.titleOne,
          sortFieldName: "crossingtype",
          width: "auto",
        },
        2: {
          name: "Пролет",
          sortFieldName: "identifiedobject",
          width: "auto",
        },
      },
      selectedAll: false,
  ...
  methods: {
    ...
    funLoadContent(page = 1) {
      ...
      axios
        .post("/api/crossingVueIndex", form)
        .then((response) => {
          // для отладки
    ...
    async funSelectedRows() {
      ...
      await axios
        .post("/api/crossingVueDelete", form)
        .then((response) => {
          // запрос прошел
          // для отладки
