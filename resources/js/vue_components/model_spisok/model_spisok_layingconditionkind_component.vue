        ...
        <tbody>
          ...
          <tr v-for="(item, index) in modelData.data" :key="index">
            ...
            <td class="text-center">
              <a :href="`layingconditionkind/edit/${item.id}`">
                {{ item.id }}
              </a>
            </td>
            <td class="text-center">
...
export default {
  name: "spisok_layingconditionkind",
  props: {
    titleOne: {
      required: true,
      type: String,
  ...
  methods: {
    ...
    funLoadContent(page = 1) {
      ...
      axios
        .post("/api/layingconditionkindVueIndex", form)
        .then((response) => {
          // для отладки
    ...
    async funSelectedRows() {
      ...
      await axios
        .post("/api/layingconditionkindVueDelete", form)
        .then((response) => {
          // запрос прошел
          // для отладки
