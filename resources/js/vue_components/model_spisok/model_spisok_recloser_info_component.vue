        ...
        <tbody>
          ...
          <tr v-for="(item, index) in modelData.data" :key="index">
            ...
            <td class="text-center">
              <a @click="saveQuery" :href="`recloser_info/edit/${item.id}`">
                {{ item.id }}
              </a>
            </td>
            <td>
  ...
  methods: {
    ...
    funLoadAll() {
      if (localStorage.getItem("table_search")) {
        if (localStorage.getItem("spisok_model") === "RecloserInfo") {
    ...
    funLoadContent(page = 1) {
      ...
      let url = `/api/modelName/RecloserInfo/switchInfo?page=${currentPage}`;
    ...
    async funSelectedRows() {
      ...
      if (Object.keys(this.selectedRows).length === 1) {
        await axios
          .delete(
            `/api/modelName/RecloserInfo/switchInfo/${this.selectedRows[0]}`
          )
          .then((response) => {
            this.funLoadAll();
          })
    ...
    saveQuery() {
      ...
      localStorage.setItem("spisok_model", "RecloserInfo");
    },
  },
};
</script>
