        ...
        <tbody>
          ...
          <tr v-for="(item, index) in modelData.data" :key="index">
            ...
            <td class="text-center">
              <a :href="`materialkind/edit/${item.id}`">
                {{ item.id }}
              </a>
            </td>
            <td class="text-center">
              <a v-if="item.name" :href="`materialkind/edit/${item.id}`">
                {{ item.name }}
              </a>
            </td>
            <td class="text-center">
...
export default {
  ...
  methods: {
    ...
    funLoadContent(page = 1) {
      ...
      axios
        .post("/api/materialkindVueIndex", form)
        .then((response) => {
          // для отладки
    ...
    async funSelectedRows() {
      ...
      await axios
        .post("/api/materialkindVueDelete", form)
        .then((response) => {
          // запрос прошел
          // для отладки
