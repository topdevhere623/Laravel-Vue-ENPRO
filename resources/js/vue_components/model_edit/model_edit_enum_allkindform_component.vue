            ...
            <div class="col-12 row">
                <div class="col-6">
                    ...
                    <div class="form-field">
                        <input
                            :disabled="loading"
                            v-model="modelData.literal"
                            class="text-field"
                            type="text"
                            required
                        />
                        <div class="form-input-label">Условное обозначение</div>
                    </div>
                    <div class="form-field">
                        <input
                            :disabled="loading"
...
export default {
    ...
    methods: {
        submit() {
            if (
                ...
                if (this.id) {
                    axios
                        .post(
                            `/api/all_enum_kind/model/${this.model}/id/${this.id}`,
                            this.modelData,
                        )
                        .then((response) => {
                            if (this.isModal) {
                                ...
                                window.location.href = '/admin/all_kind';
                            }
                    ...
                    axios
                        .post(`/api/all_enum_kind/model/${this.model}`, this.modelData)
                        .then((response) => {
                            if (this.isModal) {
                                this.$emit('onNewCreatedData', response?.data?.data || {});
                                this.$emit('save', true);
        ...
        load() {
            ...
            axios
                .get(`/api/all_enum_kind/model/${this.model}/id/${this.id}`)
                .then((r) => {
                    ...
                    this.modelData.literal = r.data.data.literal;
                    this.modelData.id = r.data.data.id;
                    console.log(r);
